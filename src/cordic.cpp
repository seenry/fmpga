...
void cordic_stage(short k, short c, short *x, short *y, short *z, int i)
{
    // inputs
    short xk = *x;
    short yk = *y;
...
void cordic(int rad, short *s, short *c, int i)
{
    short x = CORDIC_1K;
    short y = 0;
    int r = rad;
    ...
    printf("rad: %x - recalculated: %x\n", rad, z);

    for ( int k = 0; k < CORDIC_NTAB; k++ )
    {
        cordic_stage(k, CORDIC_TABLE[k], &x, &y, &z, i);
    }  
